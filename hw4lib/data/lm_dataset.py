...
class LMDataset(Dataset):
    ...
    def __init__(self, partition: str, config: dict, tokenizer: H4Tokenizer):
        ...
        # Fetch special token IDs from the tokenizer instance
        self.eos_token = tokenizer.eos_id  # EOS marks end-of-sequence
        self.sos_token = tokenizer.sos_id  # SOS marks start-of-sequence
        self.pad_token = tokenizer.pad_id  # PAD used for padding

        # Set up data paths
        # Build the path to the directory containing .npy text files
        self.text_dir = os.path.join(self.config["root"], partition)

        # Get all text files in the text directory in sorted order
        # Only consider files with .npy extension
        all_files = [f for f in os.listdir(self.text_dir) if f.endswith(".npy")]
        self.text_files = sorted(all_files)

        # Take subset if subset_size provided in config
        subset_size = self.config.get("subset_size", len(self.text_files))
        ...
        self.total_chars = 0  # total raw character count
        self.total_tokens = 0  # total token count (excluding special tokens)
        self.text_max_len = 0  # maximum sequence length seen (+1 for SOS/EOS)

        print(f"Loading transcripts for {partition} partition...")
        for file in tqdm(self.text_files):
            # Load the transcript
            # Use np.load to read array of characters and join into a string
            arr = np.load(os.path.join(self.text_dir, file), allow_pickle=True)
            transcript = "".join(arr.tolist())

            # Track character count (before tokenization)
            self.total_chars += len(transcript)

            # Use tokenizer to encode the transcript into token IDs
    ...
    def collate_fn(
        ...
        lengths = torch.LongTensor([s.size(0) for s in shifted_transcripts])
    ...
    def sample_prompts(
        self, num_samples: int, prompt_length: int, seed: int = None
