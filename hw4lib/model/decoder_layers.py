...
class SelfAttentionDecoderLayer(nn.Module):
    ...
    def __init__(self, d_model: int, num_heads: int, d_ff: int, dropout: float = 0.1):
        ...
        """
        super().__init__()
        # Instantiate masked self-attention sublayer
        self.self_attn = SelfAttentionLayer(d_model, num_heads, dropout)
        # Instantiate feed-forward sublayer
    ...
    def forward(
        ...
        """
        # Apply masked self-attention sublayer
...
class CrossAttentionDecoderLayer(nn.Module):
    ...
    def __init__(self, d_model: int, num_heads: int, d_ff: int, dropout: float = 0.1):
        ...
        # Instantiate masked self-attention sublayer
        self.self_attn = SelfAttentionLayer(d_model, num_heads, dropout)
        # Instantiate cross-attention sublayer
    ...
    def forward(
        ...
        Returns:
            x (torch.Tensor): The output tensor. shape: (batch_size, seq_len, d_model)
            self_attn_weights (torch.Tensor): The attention weights from self-attention. shape: (batch_size, seq_len, seq_len)
            cross_attn_weights (torch.Tensor): The attention weights from cross-attention. shape: (batch_size, seq_len, seq_len)
        """
        # Apply masked self-attention
        x, self_attn_weights = self.self_attn(
            x, key_padding_mask=dec_key_padding_mask, attn_mask=attn_mask
        )
        # Apply cross-attention (decoder queries attend to encoder outputs)
        x, cross_attn_weights = self.cross_attn(
            x, enc_output, key_padding_mask=enc_key_padding_mask
        )
        # Apply feed-forward sublayer
        x = self.ffn(x)
